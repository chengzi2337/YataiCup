...
# thickness - choose e.g. 100 microns for emissivity? user wants emissivity as vertical axis but for which thickness? Likely for several thicknesses similar to T plot
# We compute emissivity = 1 - R0 - T
def T_lambda_d(n, k, lambda_m, d_m):
    n_complex = n + 1j*k
    R0 = np.abs((n_complex - 1)/(n_complex + 1))**2
    alpha = 4*np.pi*k / lambda_m
    num = (1-R0)**2 * np.exp(-alpha*d_m)
    den = 1 - (R0**2)*np.exp(-2*alpha*d_m)
    return num/den, R0

d_um_list = [10, 50, 100, 200]
d_m_list = [d*1e-6 for d in d_um_list]

plt.figure()
for d_um, d_m in zip(d_um_list, d_m_list):
    T, R0 = T_lambda_d(n,k,lambda_m,d_m)
    eps = 1 - R0 - T
    plt.plot(lambda_um, eps, label=f'd={d_um} µm')

plt.xlabel("Wavelength λ (µm)")
plt.ylabel("Emissivity ε")
plt.ylim(0,1)
plt.legend()
plt.title("PDMS Emissivity vs Wavelength for Different Thicknesses")
plt.grid(True)
plt.show()
